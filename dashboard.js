...
function updateUserInterface(data, user) {
  ...
  */
  ...
  |--------------------------------------------------------------------------
  |
  | The main dashboard balance represents the wallet balance.
  | Bonus and trading profit remain separate.
  |
  */

  const totalAvailableBalance = walletBalance;
...
async function updateBitcoinMarket() {
  ...
  try {
    ...
    /*
     * Use the actual wallet balance from the
    ...
    stopUserListener =
      onSnapshot(
        ...
        (userSnapshot) => {
          ...
          /*
           * Every balance is read separately.
           *
           * balance
           * fundingBalance
           * bonusBalance
